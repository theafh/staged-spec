...
Read all files under `/specs` — `architecture.md`, `intent.md`, `security.md`, `testing.md`, `features.md`, and every stage spec file. Treat stage files marked `implemented` or `in progress` as historical records and context: use them for cross-file checks (status alignment, dependencies, terminology, and link validity), and focus stage-level quality issues strictly on stage specs that have the status `planned`. Cross-reference the framework against the assessment dimensions defined in the framework_assessment reference.
